...
noise_dim = 4   # <- size of input noise
lr = 0.0001  # <- 0.0001
beta = "s0018"
...
savedModel = "savedModels/isingOne(" + str(latent_dim) + "-" + beta + ")Linear[" + str(
    noise_dim) + "]128.pth"  # <- path to save model
savedDataPath = "outIsingData/" + beta + "_128/" + str(latent_dim) + "-" + beta + "[" + str(
    noise_dim) + "]_128/outputData(" + str(
    latent_dim) + "-" + beta + ")_128TestFileLinear[" + str(noise_dim) + "].npy"  # <-path to save the data
