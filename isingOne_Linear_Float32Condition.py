...
ising_data = np.load('ising/s_cfg_x032_b0150.npy')  # <- input ising model configuration
...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.main = nn.Sequential(
            ...
            nn.Linear(in_features=noise_dim + condition_dim, out_features=128),
            nn.LeakyReLU(0.2),
    ...
    def forward(self, noise, condition):
        input = torch.cat((noise, condition), dim=1)  # <- connect noice and condition
...
def random_noise(batch_size, noise_dim, condition_dim):  # <- Dodanie nowego parametru wejściowego
    noise = torch.rand(batch_size, noise_dim)
    condition = torch.rand(batch_size, condition_dim)  # <- Losowanie warunków
...
def gen_noise(b_size):
    ...
    for i in range(b_size):
        ...
        float32_num_temp = list(''.join('{:0>8b}'.format(c) for c in struct.pack('!f', num)))
        float32_num = [int(i) for i in float32_num_temp]
        batch.append(float32_num)
    numpy_batch = np.array(batch)
    out_tensor = torch.from_numpy(numpy_batch).float()
    # out_tensor = torch.unsqueeze(out_tensor, dim=1)
    # _out_tensor = out_tensor.type(torch.LongTensor)
    return out_tensor
